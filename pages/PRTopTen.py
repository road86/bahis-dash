...
def TopTen(sub_bahis_sourcedata, bahis_dgdata, distype, to_replace, replace_with):
    ...
    fpoul.update_layout(
        height=250, margin={"r": 0, "t": 0, "l": 0, "b": 0}, xaxis=dict(minallowed=0, tickformat=",.0f")
    )
    ...
    flani = px.bar(
        ...
        x="counts",
        y="top_diagnosis",
        labels={"counts": "Number of Reports", "top_diagnosis": ""},
        title="",
    )
    flani.update_layout(
        height=250, margin={"r": 0, "t": 0, "l": 0, "b": 0}, xaxis=dict(minallowed=0, tickformat=",.0f")
    ...
    if distype:
        ...
        figDisTyp.update_layout(
            height=200, margin={"r": 0, "t": 0, "l": 0, "b": 0}, xaxis=dict(minallowed=0, tickformat=",.0f")
        )
    else:
        figDisTyp = {}
