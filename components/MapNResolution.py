...
def plotMap(geoResolution, geoResolutionNo, reportsdata, geoNameNNumber, shapePath):
    ...
    reports = reports.rename(columns = {geoResolution: "cases"})
    ...
    if geoResolutionNo == 1:
        reports["divisionname"] = reports["divisionname"] + " Division"
        print(reports)
    if geoResolutionNo == 2:
        print(reports)

    custolor = [[0, "white"], [1 / reports["cases"].max(), "lightgray"], [1, "red"]]
    fig = px.choropleth_mapbox(
        ...
        range_color=(1, reports["cases"].max()),
        mapbox_style="carto-positron",
        zoom=5.8,
        center={"lat": 23.7, "lon": 90.3},
        opacity=0.7,
        labels={"cases": "Reports"},
